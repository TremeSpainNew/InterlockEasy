...
async function test(page){
 ...
 assert.equal(names.includes('debounceMs'),page==='inputs');
 assert.equal(names.includes('commandTopic'),page==='outputs');
 assert.equal(controls.some(n=>n.textContent==='Añadir señal'),page==='signals');
 config.mqtt.host='newer-broker';config.inputs[0].name='Changed elsewhere';
 await elements['config-form'].handlers.submit({preventDefault(){}});
 assert.ok(submitted);
 if(page!=='mqtt')assert.equal(submitted.mqtt.host,'newer-broker');
 if(page!=='inputs')assert.equal(submitted.inputs[0].name,'Changed elsewhere');
 assert.equal('password' in submitted.mqtt,false);
}
(async()=>{
 for(const page of ['mqtt','inputs','outputs','signals'])await test(page);
